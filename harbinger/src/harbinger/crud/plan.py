...
async def get_implant_statistics(db: AsyncSession) -> dict:
    ...
    return {
        "items": [{"key": key, "value": value, "icon": "question_mark"} for key, value in stats.items()],
    }
...
async def get_plan(db: AsyncSession, id: UUID4 | str) -> models.Plan | None:
