...
async def get_highlights_filters(db: AsyncSession, filters: filters.HighlightFilter) -> list[schemas.Filter]:
    result: list[schemas.Filter] = []
    q: Select = (
...
async def create_llm_log(db: AsyncSession, llm_log: schemas.LlmLogCreate) -> models.LlmLog:
...
async def update_llm_log(db: AsyncSession, id: str | uuid.UUID, llm_logs: schemas.LlmLogUpdate) -> None:
    q = update(models.LlmLog).where(models.LlmLog.id == id).values(**llm_logs.model_dump(exclude_unset=True, exclude_defaults=True, exclude_none=True))
    await db.execute(q)
    await db.commit()
