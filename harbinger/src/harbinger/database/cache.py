...
OriginalUpdateFuncType = Callable[
    Concatenate[AsyncSession, P], Awaitable[Optional[Any]]
]
WrappedUpdateFuncType = Callable[
    Concatenate[AsyncSession, P], Awaitable[Optional[SchemaType]]
]
    ...
    def decorator(
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Optional[SchemaType]:
            ...
            async with session_factory() as db:
                ...
                if db_result is None:
                    return (
                        None  # Not found, return None, don't cache absence by default
                    )
...
def redis_cache_update(
    ...
    def decorator(func: OriginalUpdateFuncType) -> WrappedUpdateFuncType:
        ...
        async def wrapper(
            db: AsyncSession, *args: P.args, **kwargs: P.kwargs
        ) -> Optional[SchemaType]:
...
def redis_cache_fixed_key(
    ...
    def decorator(func: OriginalFixedKeyFuncType) -> WrappedFixedKeyFuncType:
        ...
        @functools.wraps(func)
        async def wrapper() -> Optional[
            SchemaType
        ]:  # Wrapper takes no arguments directly
