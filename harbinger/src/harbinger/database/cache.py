...
OriginalCacheFuncType = Callable[Concatenate[AsyncSession, P], Awaitable[Any | None]]
...
WrappedCacheFuncType = Callable[P, Awaitable[SchemaType | None]]

OriginalUpdateFuncType = Callable[
    Concatenate[AsyncSession, P],
    Awaitable[Any | None],
]
WrappedUpdateFuncType = Callable[
    Concatenate[AsyncSession, P],
    Awaitable[SchemaType | None],
]
...
def redis_cache(
    ...
    def decorator(
        func: OriginalCacheFuncType[P],
    ) -> WrappedCacheFuncType[P, SchemaType]:
        ...
        if not params or params[0] != "db":  # Assuming 'db' is the conventional name
            logger.warning(
                f"Function {func.__name__} decorated with session-managing redis_cache "
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> SchemaType | None:
            ...
            try:
                ...
                logger.exception(
                    f"Error extracting key param '{key_param_name}' for {func.__name__}: {e}",
                )
            ...
            if key_value is None:
                logger.warning(
                    f"Key parameter '{key_param_name}' not found in args/kwargs for {func.__name__}. Skipping cache.",
                )
                ...
                async with session_factory() as db:
                    ...
                    try:
                        ...
                        logger.exception(
                            f"Failed to convert result to schema {schema.__name__} for uncached call: {e}",
                        )
            ...
            try:
                ...
                if cached_data:
                    try:
                        ...
                        logger.exception(
                            f"Failed to load/validate cached data for {cache_key}: {e}",
                        )
                        # Treat as cache miss if data is corrupted
            except RedisError as e:
                logger.exception(
                    f"Redis GET error for key {cache_key}: {e}. Falling back to function call.",
                )
            ...
            async with session_factory() as db:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to convert DB result to schema {schema.__name__} for key {cache_key}: {e}",
                    )
            ...
            try:
                ...
                logger.exception(f"Redis SET error for key {cache_key}: {e}")
            except Exception as e:
                logger.exception(
                    f"Failed to serialize schema {schema.__name__} for caching key {cache_key}: {e}",
                )
...
def redis_cache_update(
    ...
    def decorator(func: OriginalUpdateFuncType) -> WrappedUpdateFuncType:
        ...
        if not params or params[0] != "db":  # A check for convention
            logger.warning(
                f"Function {func.__name__} decorated with redis_cache_update "
                f"does not seem to have 'db' as its first parameter. Ensure it expects "
                f"an AsyncSession as the first argument.",
            )
            ...
            **kwargs: P.kwargs,
            ...
            try:
                ...
                bound_args = sig.bind_partial(
                    ...
                    *args,
                    **kwargs,
                )  # Use bind_partial before applying defaults
                ...
                logger.exception(
                    f"Error binding/extracting key param '{key_param_name}' for update function {func.__name__}: {e}",
                )
                key_value = None
            ...
            if can_cache:
                ...
                logger.warning(
                    f"Key parameter '{key_param_name}' not found or extractable for {func.__name__}. Update will proceed, but cache will not be updated.",
                )
                cache_key = None  # Ensure cache_key is None if we can't cache

            # 1. Call the original update function (MUST always be called)
            updated_db_result: Any | None = None
            schema_result: SchemaType | None = None
            try:
                ...
                if updated_db_result is None:
                    logger.warning(
                        f"Update function {func.__name__} returned None. Assuming object not found or update failed.",
                    )
                ...
                try:
                    ...
                    logger.exception(
                        f"Failed to convert updated DB result to schema {schema.__name__} "
                ...
                if can_cache and cache_key and schema_result:
                    try:
                        ...
                        await redis.set(cache_key, data_to_cache, ex=ttl_seconds)
                        logger.info(
                            f"Successfully updated cache for key: {cache_key} with TTL: {ttl_seconds}s",
                        )
                    except RedisError as e:
                        logger.exception(
                            f"Redis SET error while updating cache for key {cache_key}: {e}",
                        )
                        # Log error, but proceed to return the schema_result as DB was updated.
                    except Exception as e:
                        logger.exception(
                            f"Failed to serialize schema {schema.__name__} for caching update key {cache_key}: {e}",
                        )
                        # Log error, proceed to return schema_result.
                elif not can_cache:
                    logger.warning(
                        f"Skipped cache update for {func.__name__} because key parameter was not found.",
                    )
                ...
                logger.error(
                    f"Error during database update operation in {func.__name__}: {db_update_error}",
                    exc_info=True,
                )
...
def redis_cache_invalidate(
    ...
    def decorator(func: OriginalInvalidateFuncType) -> WrappedInvalidateFuncType:
        ...
        if not params or params[0] != "db":
            logger.warning(
                f"Function {func.__name__} decorated with redis_cache_invalidate "
                f"does not seem to have 'db' as its first parameter. Ensure it expects "
                f"an AsyncSession as the first argument.",
            )
            ...
            try:
                ...
                if can_invalidate and cache_key:
                    try:
                        ...
                        logger.exception(
                            f"Redis DEL error while invalidating cache key {cache_key}: {e}",
                        )
                return db_result
...
async def invalidate_cache_entry(key_prefix: str, key_value: Any) -> bool:
    ...
    if key_value is None:
        logger.warning(
            f"Attempted to invalidate cache for prefix '{key_prefix}' with None key_value. Aborting.",
        )
        return False
...
WrappedFixedKeyFuncType = Callable[[], Awaitable[SchemaType | None]]
...
def redis_cache_fixed_key(
    ...
    def decorator(func: OriginalFixedKeyFuncType) -> WrappedFixedKeyFuncType:
        ...
        if not params or params[0] != "db" or len(params) > 1:
            logger.warning(
                f"Function {func.__name__} decorated with fixed-key cache "
                f"should ideally only accept 'db' as an argument. Found: {params}",
            )

        @functools.wraps(func)
        async def wrapper() -> SchemaType | None:  # Wrapper takes no arguments directly
            ...
            try:
                ...
                if cached_data:
                    ...
                    try:
                        ...
                        logger.exception(
                            f"Failed to load/validate cached data for fixed key {cache_key}: {e}",
                        )
                        # Treat as cache miss
            except RedisError as e:
                logger.exception(
                    f"Redis GET error for fixed key {cache_key}: {e}. Falling back.",
                )
            ...
            try:
                async with session_factory() as db:
                    ...
                    try:
                        ...
                        logger.exception(
                            f"Failed to convert result to schema {schema.__name__} for fixed key {cache_key}: {e}",
                        )
                        return None  # Return None if conversion fails

            except Exception as call_exc:
                logger.error(
                    f"Error executing function {func.__name__} for fixed key {cache_key}: {call_exc}",
                    exc_info=True,
                )
            ...
            if schema_result is not None:
                try:
                    ...
                    await redis.set(cache_key, data_to_cache, ex=ttl_seconds)
                    logger.debug(
                        f"Cached data for fixed key: {cache_key} with TTL: {ttl_seconds}s",
                    )
                except RedisError as e:
                    logger.exception(f"Redis SET error for fixed key {cache_key}: {e}")
                except Exception as e:
                    logger.exception(
                        f"Failed to serialize schema {schema.__name__} for caching fixed key {cache_key}: {e}",
                    )
...
Neo4jSessionFactoryType = Callable[[], AbstractAsyncContextManager[Neo4jAsyncSession]]
...
WrappedNeo4jFuncType = Callable[[], Awaitable[SchemaType | None]]
...
def redis_cache_neo4j_cm_fixed_key(
    ...
    def decorator(func: OriginalNeo4jFuncType) -> WrappedNeo4jFuncType:
        ...
        if not params:
            logger.error(
                f"Function {func.__name__} decorated with Neo4j session-managed cache "
        ...
        async def wrapper() -> SchemaType | None:
            # 1. Check cache using the fixed key
            try:
                cached_data = await redis.get(cache_key)
                if cached_data:
                    ...
                    try:
                        ...
                        logger.exception(
                            f"CACHE CORRUPTION: Failed to load/validate cached data for Neo4j fixed key {cache_key}: {e}",
                        )
                        # Treat as cache miss
            except RedisError as e:
                logger.exception(
                    f"REDIS ERROR (GET): Failed for Neo4j key {cache_key}: {e}. Falling back to DB.",
                )
            except Exception as e:
                logger.error(
                    f"UNEXPECTED ERROR during cache check for {cache_key}: {e}",
                    exc_info=True,
                )

            # 2. Cache Miss: Create session and call original function
            logger.debug(
                f"Cache MISS for Neo4j fixed key: {cache_key}. Calling {func.__name__}.",
            )
            schema_result: SchemaType | None = None
            try:
                ...
                async with session_factory() as session:
                    ...
                    if db_result is None:
                        logger.info(
                            f"{func.__name__} returned None for Neo4j fixed key {cache_key}. Result not cached.",
                        )
                    ...
                    try:
                        ...
                        logger.exception(
                            f"SCHEMA VALIDATION FAILED: Could not convert Neo4j result from {func.__name__} "
                ...
                logger.error(
                    f"NEO4J/FUNCTION ERROR: Failed during session creation or execution of {func.__name__} "
                    f"for fixed key {cache_key}: {call_exc}",
                    exc_info=True,
                )
            ...
            if schema_result is not None:
                try:
                    ...
                    await redis.set(cache_key, data_to_cache, ex=ttl_seconds)
                    logger.debug(
                        f"Cached Neo4j data for fixed key: {cache_key} with TTL: {ttl_seconds}s",
                    )
                except RedisError as e:
                    logger.exception(
                        f"REDIS ERROR (SET): Failed for Neo4j key {cache_key}: {e}",
                    )
                    # Log error, but still return the result obtained from DB
                except Exception as e:
                    logger.exception(
                        f"SERIALIZATION ERROR: Failed to serialize schema {schema.__name__} "
