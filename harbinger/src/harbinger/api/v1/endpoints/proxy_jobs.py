...
async def get_proxy_job(job_id: str, user: models.User = Depends(current_active_user)):
    return await crud.get_proxy_job(job_id=job_id)


@router.post(
    "/{job_id}/kill", response_model=schemas.ProxyJob, tags=["proxy_jobs", "crud"]
)
async def kill_proxy_job(job_id: str, user: models.User = Depends(current_active_user)):
    return await crud.get_proxy_job(job_id=job_id)
...
async def start_proxy_job(
    job_id: str, response: Response, user: models.User = Depends(current_active_user)
):
...
@router.post(
    "/{job_id}/clone", response_model=schemas.ProxyJob, tags=["proxy_jobs", "crud"]
)
