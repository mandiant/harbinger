...
@router.get(
    "/filters", response_model=list[schemas.Filter], tags=["credentials", "crud"]
)
...
@router.get(
    "/{credential_id}", response_model=schemas.Credential, tags=["credentials", "crud"]
)
async def read_credential(
    credential_id: str, user: models.User = Depends(current_active_user)
):
