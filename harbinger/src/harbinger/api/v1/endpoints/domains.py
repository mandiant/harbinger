...
@router.get("/{id}", response_model=Optional[schemas.Domain], tags=["crud", "domains"])
async def get_domain(id: UUID4, user: models.User = Depends(current_active_user)):
...
@router.put("/{id}", response_model=Optional[schemas.Domain], tags=["crud", "domains"])
