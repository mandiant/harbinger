...
async def upload_single_file(path: Path, socks_job_id: str):
...
async def upload_main(path: str, exclude: list[str] = [], socks_job_id: str = '') -> None:
    p = Path(path)
    to_upload = []
    if p.is_dir():
        for x in p.iterdir():
    ...
    for u in to_upload:
        await upload_single_file(u, socks_job_id)
...
def upload(path: str, exclude: tuple[str], socks_job_id: str):
    anyio.run(upload_main, path, list(exclude), socks_job_id)
...
def download(files: tuple[str], destination: str, bucket: str):
    anyio.run(download_main, files, destination, bucket)
