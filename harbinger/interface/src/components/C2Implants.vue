...
<template>
  <div>
    ...
    <q-table :rows-per-page-options="[ 5, 10, 15, 20, 25, 50, 100 ]" title="Implants" :rows="data" row-key="id" :columns="columns" :loading="loading"
      ...
      <template v-slot:top>
        <div class="col-2 q-table__title">Implants</div>
        <q-space />
        <filter-view object-type="c2/implants" v-model="filters" v-on:updateFilters="store.updateFilters" />
...
const useStore = defineTypedStore<C2Implant>('c2/implants');
