...
<template>
  <div>
    ...
    <q-table :rows-per-page-options="[5, 10, 15, 20, 25, 50, 100]" title="Highlight" :rows="data" row-key="id"
      ...
      <template v-slot:top>
        <div class="col-2 q-table__title">Highlights</div>
        <q-space />
        <filter-view object-type="highlights" v-model="filters" v-on:updateFilters="store.updateFilters" />
        <q-select v-model="visible" multiple borderless dense options-dense :display-value="$q.lang.table.columns"
          ...
          <template v-slot:option="{ itemProps, opt, selected, toggleOption }">
            <q-item v-bind="itemProps">
              <q-item-section>
                <q-item-label :class="$q.dark.isActive ? 'text-white' : 'text-black'">{{ opt.label
                  }}</q-item-label>
              </q-item-section>
              <q-item-section side>
