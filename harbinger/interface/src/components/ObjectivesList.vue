...
<div>
    ...
    <template v-slot:top>
        <div class="col-2 q-table__title" v-if="selected.length === 0">Objectives</div>
        <div v-if="selected.length > 0" class="row items-center q-gutter-sm">
            <bulk-label-actions :selected="selected" object-type="objective" @update="selected = []; store.LoadData()" />
        ...
        <q-space />
        <filter-view v-if="selected.length === 0" object-type="objectives" v-model="filters" v-on:updateFilters="store.updateFilters" />
